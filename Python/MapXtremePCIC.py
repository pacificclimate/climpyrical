...
class MapXtremePCIC:
    """
    Documentation from R code

    MapXtremePCIC maps design values over North America
    ====================================================
    Attributes
     CanRCM4.lens : information data list for CanRCM4
     modelled design values over North America
     obs : data array of observed design values
     over North America, [lon, lat, data] three columns
     res : resolution (in km) of the map
     method : whether EOF or SOM-based method
      is employed for mapping
    ...
    Value
     ...
     sp.basis : data array of the spatial basis functions
     estimated from CanRCM4 modelled data
     obs.grid : data array of the gridded observations

    Note: the coordinate system is in polar rotated
     projection for all involved arrays. The projection
     is "+proj=ob_tran +o_proj=longlat +lon_0=-97
     +o_lat_p=42.5 +a=1 +to_meter=0.0174532925199 +no_defs"

    Author : Nic Annau at PCIC, University of Victoria, nannau@uvic.ca
    """

    def __init__(self, res, method, data_path, variable,  R=6371.0):
        """Declare and check arguments of MapXtremePCIC object

        Args:
            res (str): resolution used to draw maps
            method (str): statistical reconstruction method to use
            data_path (str): path to ensemble of CanRCM4 models
            variable (str): design value used for mapping - must
                            match the CanRCM4 models provided
            R (float): radius of earth used to area calculations
        Returns: 
            None
        """
        ...
        if isinstance(data_path, str) is False:
            raise ValueError('Method argument requires {} got {}'
                             .format(type('string'), type(data_path)))

        if isinstance(method, str) is False:
            raise ValueError('Method argument requires {} got {}'
                             .format(type('string'), type(method)))

        if (method != 'eof' and method != 'som'):
            raise Exception('MapXtremePCIC requires specified \'som\' \
                             or \'eof\'. Got {}'
                            .format(method))

        # Check map resolution type
        if isinstance(res, str) is False:
            raise ValueError('Mapping resolution requires {}, got {}'
                             .format(str, type(res)))
        ...
        if res not in ('10m', '50m', '110m'):
            raise Exception('Requires res to be 10m, 50m or 110m')

    def read_data(self):
        """
        Returns:
            ds (xarray data cube): data cube of assembled ensemble models 
    ...
    def grid_area(self):
        """
        Returns array of areas for grid space, calculated
        from rectangular section on the sphere.
        ...
        lat_sz = np.abs(np.diff(np.sin(np.deg2rad(ds['lat'].values)),
                        axis=0)[:, :-1])
        lon_sz = np.abs(np.deg2rad(np.diff(ds['lon'].values,
                        axis=1)[:-1, :]))
    ...
    def get_arr(self, df=None):
        """
        Converts pandas dataframe containing runs as columns
        organized from dataset to n x p array

        Parameters
        ----------
        pandas dataframe : full dataframe with each run
        labelled as in MapXtremePCIC.get_df()
    ...
    def get_df(self, ds=None):
        ...
        Parameters
        ----------
        xarray Dataset : Data cube with geospatial and
        field data for ensemble of CanRCM4 data
    ...
    def weight_matrix(self):
        ...
        Parameters
        ----------
        xarray dict : Data cube with geospatial and field data
        for ensemble of CanRCM4 data
    ...
    def ensemble_mean(self, df=None, reverse=False):
        ...
        Parameters
        ----------
        pandas dataframe : Data cube with geospatial and
        field data for ensemble of CanRCM4 data
        ...
        if df is None:
            if reverse is True:
                warnings.warn("Incorrect mean if previously \
                              corrected dataframe not provided.")
    ...
    def standard_matrix(self, df=None, reverse=False, mean_run=None):
        """
        Returns weighted and standardized array using
        fractional grid cell areas
        ...
        if df is None:
            if reverse is True:
                warnings.warn("Incorrect mean if you do not provide\
                              a previously corrected dataframe")

            df = self.ensemble_mean(reverse=False)

        df_copy, df_return = df.copy(), df.copy()
    ...
    def plot_reference(self, df=None, run='run0',
                       plot_title='50-year daily precipitation [mm/h]',
                       save_fig=False):
        """
        ...
        """
        # Ignore NaN values for plotting irrelevant warnings
        np.seterr(invalid='ignore')
    ...
    def sample(self, df=None, frac=0.02, seed=None, dropna=True):
        ...
        int   : random state which to use. Use the same integer
                to preserve sample that is randomly sampled between
                calls to the function. If None, returns a new random
                sample each call to the function.
    ...
    def get_pseudo_obs(self, frac=0.02, seed=True):
        """
        Returns randomly pseudo observations from CanRCM4 models

        Parameters
        ----------
        float : fraction of data to return
        int   : random state which to use. Use the same integer
                to preserve sample that is randomly sampled
                between calls to the function. If None, returns
                a new random sample each call to the function.

        Returns
        -------
        out : pandas df of pseudo observations
        """
        ...
        df_pseudo = self.ensemble_mean(df=df)\
                        .dropna()\
    ...
    def combine_model_obs(self, df_obs, df_model):
        ...
        df_result = df_model.merge(df_obs,
                                   how='outer',
                                   on=['rlat', 'rlon']).copy()
        rlat_diff = abs(df_result["rlat"]-df_result["rlat_copy"])
        rlon_diff = abs(df_result["rlon"]-df_result["rlon_copy"])

        rlat_tol = rlat_diff >= tolerance_rlat
        rlon_tol = rlon_diff >= tolerance_rlon

        df_result.loc[rlat_tol, :] = np.nan
        df_result.loc[rlon_tol, :] = np.nan
    ...
    def plot_scatter(MapXtreme, frac, run=0, seed=True):
        ...
        # define projections
        ax = plt.axes(projection=rp)
        ax.set_title('50-year daily precipitation [mm/h]',
                     fontsize=30,
        ...
        colorplot = ax.scatter(df['rlon'], df['rlat'],
                               c=run_obs, cmap=cmap,
                               transform=rp, vmin=1.,
                               vmax=13.)
