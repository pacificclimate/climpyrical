...
@pytest.mark.parametrize(
    ...
    [
        (
            resource_filename('tests', 'snw.nc'),
            'snw',
            {'rlat', 'rlon', 'lat', 'lon'},
            (66, 130, 155),
        ),
        (
            resource_filename('tests/data/', 'hdd.nc'),
            'heating_degree_days_per_time_period',
            {'rlat', 'rlon', 'lat', 'lon', 'level'},
            (35, 130, 155),
        ),
        (
            resource_filename('tests', 'example1.nc'),
            'hyai',
            {'lon', 'lat'},
            (27,),
        ),
        (
            resource_filename('tests', 'example2.nc'),
            'tas',
            {'lat', 'lon'},
            (1, 130, 155),
        ),
        (
            resource_filename('tests', 'example3.nc'),
            'tas',
            {'lat', 'lon'},
            (1, 128, 256),
        ),
        (
            resource_filename('tests', 'example4.nc'),
            'tos',
            {'lat', 'lon'},
            (24, 170, 180),
        ),
