...
from climpyrical.datacube import read_data, check_keys
...
@pytest.mark.parametrize('data_path,design_value_name,keys,shape', [
    (os.path.join(THIS_DIR, './data/snw.nc'), 'snw', {'rlat', 'rlon', 'lat', 'lon'}, (66, 130, 155)),
    (os.path.join(THIS_DIR, './data/hdd.nc'), 'heating_degree_days_per_time_period', {'rlat', 'rlon', 'lat', 'lon', 'level'}, (35, 130, 155)),
    (os.path.join(THIS_DIR, './data/example1.nc'), 'hyai', {'lon', 'lat'}, (27,)),
    (os.path.join(THIS_DIR, './data/example2.nc'), 'tas', {'lat', 'lon'}, (1, 130, 155)),
    (os.path.join(THIS_DIR, './data/example3.nc'), 'tas', {'lat', 'lon'}, (1, 128, 256)),
    (os.path.join(THIS_DIR, './data/example4.nc'), 'tos', {'lat', 'lon'}, (24, 170, 180))])
